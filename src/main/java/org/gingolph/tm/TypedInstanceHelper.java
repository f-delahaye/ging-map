...
public class TypedInstanceHelper {
  public static <T extends Construct & TypedConstruct> void setType(T typed, Topic type,
      Consumer<Topic> setter) {
    if (type == null) {
    ...
    setter.accept(type);
    ((TopicMapImpl) typed.getTopicMap())
        .notifyListeners(listener -> listener.onTypeChanged(typed, type, oldType));
  }
}
