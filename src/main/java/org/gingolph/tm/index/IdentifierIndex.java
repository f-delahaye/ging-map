...
/**
 * Not directly exposed as an index by tmapi, but exposed through TopicMap.getBy*Identifiers() methods.
 */
public class IdentifierIndex extends AbstractIndex implements Index, TopicMapEventListener {
    ...
    public IdentifierIndex(TopicMap topicMap, Collection<Topic> topics, Collection<Association> associations) {
        ...
        topics.forEach(topic-> {
                    ...
                    topic.getSubjectIdentifiers().forEach(identifier->onSubjectIdentifierAdded(topic, identifier));
        });
    ...
    public final void onConstructCreated(Construct construct) {
        ...
        construct.getItemIdentifiers().forEach(identifier->onItemIdentifierAdded(construct, identifier));        
    }
