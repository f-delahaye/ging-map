...
public final class ScopedIndexImpl extends AbstractIndex implements ScopedIndex, TopicMapEventListener {
     ...
     * @param <T>
     * @param scoped
     * @param declaredScopedClass the tm api (and not its concrete gingolp implementation). It has to match the class used when looking up a scope, which is the tm api interface.
     */
    ...
    protected <T extends Scoped> Scope<T> getOrCreateScope(Class<T> scopedClass) {
        if (!(scopedClass.getPackage().getName().equals("org.tmapi.core"))) {
            ...
            try {
                scopedClass = (Class<T>) Class.forName("org.tmapi.core."+scopedClassName.substring(0, scopedClassName.length() - "Impl".length()));
            } catch (Exception exc) {
    ...
    private static class Scope<T extends Scoped> {
        ...
        public Collection<T> getScopeds(Topic theme) {
            ...
            return scopeds == null?Collections.emptyList():Collections.unmodifiableCollection(scopeds);
        }
        
        protected Collection<T> getScopeds(Topic[] themes, boolean matchAll) throws IllegalArgumentException {
            return AbstractIndex.getPropertiedObjects(themesToScopeds, Scoped::getScope, themes, matchAll);
        }    
