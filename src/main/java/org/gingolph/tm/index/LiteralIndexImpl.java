...
public class LiteralIndexImpl extends AbstractIndex implements LiteralIndex, TopicMapEventListener {
    ...
    protected <T extends DatatypeAware> Collection<T> getDatatypeAwares(Class<T> valuedClass, String value, Locator datatype) throws IllegalArgumentException {
        if (value == null) {
        ...
        return getValued(value, valuedClass).stream().filter(occurrence -> occurrence.getDatatype() == datatype).collect(Collectors.toList());
    }
    ...
    protected <T> Collection<T> getValued(String value, Class<T> valuedClass) {
        ...
        if (collection != null) {
            return collection.stream().filter(valued->valuedClass.isAssignableFrom(valued.getClass())).map(valued -> valuedClass.cast(valued)).collect(Collectors.toList());
        }
