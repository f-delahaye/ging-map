...
public class HGScopedIndex extends HGAbstractIndex implements ScopedIndex {
    ...
    private static class Unscoped implements HGAtomPredicate, HGQueryCondition {
        ...
        public boolean satisfies(HyperGraph graph, HGHandle handle) {
            ...
            return atom instanceof HGScopedSupport && org.assertj.core.util.Collections.isNullOrEmpty(((HGScopedSupport)atom).getScope());
        }
    }    
    ...
    protected <T extends Scoped, S extends HGScopedSupport<T>> Collection<T> getScoped(Topic theme, Class<S> scopedSupportClass) {
        Collection<T> scoped;
        if (theme == null) {
            // all unscoped associations
            scoped = hg.<T>findAll(graph, 
                    hg.apply(supportHandleToOwnerMapping(graph, scopedSupportClass),
    ...
    protected <T extends Scoped, S extends HGScopedSupport<T>> Collection<T> getScoped(Topic[] themes, boolean matchAll, Class<S> scopedSupportClass) {
        if (themes == null) {
        ...
        for (Topic theme: themes) {
            ...
            HGQueryCondition themeQuery = hg.and(hg.type(HGTM.hScopeOf),
                    hg.incident(themeHandle),
                    hg.orderedLink(hg.anyHandle(), themeHandle),
                    linkTargetSubsumed(0, scopedSupportClass));
            if (relQuery == null) {
                relQuery = themeQuery;
            } else {
        ...
        scopedConstructs = hg.findAll(graph,
                hg.apply(supportHandleToOwnerMapping(graph, scopedSupportClass),
    ...
    protected <T extends Scoped> Collection<T> filterMatchAll(Collection<T> scopedConstructs, Topic[] themes) {
        ...
        scopedConstructs.forEach(scopedConstruct -> scopedCount.compute(scopedConstruct, (key, currentCount) -> currentCount == null ? 1:++currentCount));
        ...
        scopedConstructs = scopedCount.entrySet().stream().filter(entry -> entry.getValue() == themes.length).map(entry -> entry.getKey()).collect(Collectors.toList());
        return scopedConstructs;
    }

    protected <S extends HGScopedSupport<?>> Collection<Topic> getScopedThemes(Class<S> scopedSupportClass) {
        HGQueryCondition relQuery = hg.and(hg.type(HGTM.hScopeOf), linkTargetSubsumed(0, scopedSupportClass));
        List<HGHandle> themeSupports = hg.findAll(graph, relQuery);
        return themeSupports.stream().map(handle -> graph.<HGLink>get(handle)).map(link -> graph.<HGTopicSupport>get(link.getTargetAt(1)).getOwner()).collect(Collectors.toList());
    }
    ...
    public Collection<Variant> getVariants(Topic theme) {
        ...
        allVariants.addAll(getScoped(theme, HGNameSupport.class).stream().flatMap(name -> name.getVariants().stream()).collect(Collectors.toList()));
        return allVariants;
    }
    ...
    public Collection<Variant> getVariants(Topic[] themes, boolean matchAll) {
        ...
        allVariants.addAll(getScoped(themes, false, HGNameSupport.class).stream().flatMap(name -> name.getVariants().stream()).collect(Collectors.toSet()));
