...
//    public static HGHandleFactory HANDLE_FACTORY = new UUIDHandleFactory();
    ...
    static HGTopicMapSupport getTopicMapOf(HyperGraph graph, HGHandle topicMapItemHandle) {
        HGHandle rel = hg.findOne(graph,
                hg.and(hg.type(HGTM.hMapMember),
                        hg.incident(topicMapItemHandle),
                        hg.orderedLink(topicMapItemHandle, hg.anyHandle())));
        if (rel == null) {
            return null;
        } else {
    ...
    static void setTopicMapOf(HyperGraph graph, HGHandle topicMapItemHandle, HGHandle topicMapHandle) {
        graph.add(new HGRel(HGTM.MapMember, new HGHandle[]{topicMapItemHandle, topicMapHandle}),
                HGTM.hMapMember);
    }
    
    static <C extends Construct, T extends HGConstructSupport<C>> Set<C> findTopicMapItems(HyperGraph graph, Class<T> topicMapItemClass, HGHandle topicMapHandle) {
            ...
            Collection<HGHandle> handles = graph.findAll(hg.and(hg.type(topicMapItemClass),
                    hg.apply(hg.linkProjection(0),
                            hg.apply(hg.deref(graph),
                                    hg.and(hg.type(HGTM.hMapMember),
                                            hg.incident(topicMapHandle))))));
            return handles.stream().map(handle -> graph.<T>get(handle)).map(support -> support.getOwner()).collect(Collectors.toSet());
    }
    ...
    static void setTypeOf(HyperGraph graph, HGHandle type, HGHandle typed) {
        HGHandle rel = hg.findOne(graph,
                hg.and(hg.type(HGTM.hTypeOf),
                        hg.incident(typed),
                        hg.orderedLink(hg.anyHandle(), typed)));
        if (rel != null) {
            graph.remove(rel);
        }
        if (type != null) {
    ...
    static <C extends Construct> Set<C> getRelatedObjects(HGConstructSupport support, HGHandle relType, boolean supportAsFirstArgument) {
        HyperGraph graph = support.getGraph();
        HGHandle supportHandle = getHandle(graph, support);
        if (supportHandle == null) {
            return null;
        }
        Set<HGConstructSupport<C>> relatedSupports = HGTMUtil.getRelatedObjects(graph, relType, supportAsFirstArgument?supportHandle:null, supportAsFirstArgument?null:supportHandle);        
        return relatedSupports.stream().map(relatedSupport -> relatedSupport.getOwner()).collect(Collectors.toSet());
    }
    
    /**
     * Get all 'first' related to a given 'second' in an ordered link or,
     * vice-versa, all 'second' related to a given 'first' in an ordered link.
     * Whether first or second is required is indicated by putting null in the
    ...
    /**
     * Get the 'first' related to a given 'second' in an ordered link or,
     * vice-versa, the 'second' related to a given 'first' in an ordered link.
     * Whether first or second is required is indicated by putting null in the
     * corresponding parameter.
     */
    ...
    static void detachFromMap(HyperGraph graph, HGHandle topicMapItemToDetach) {
        List<HGHandle> all = hg.findAll(graph, hg.and(hg.type(HGTM.hMapMember),
                hg.incident(topicMapItemToDetach),
                hg.orderedLink(new HGHandle[]{
                    topicMapItemToDetach,
                    hg.anyHandle()})));
