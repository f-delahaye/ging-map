...
public class HGRoleSupport extends HGConstructSupport<Role> implements RoleSupport, HGLink {
    ...
    private HGRoleSupport(Role owner, HGHandle [] targetSet) {
        ...
        // usually graph is injected to Constructs when they get persisted (by virtue of them implementing HoldingContainer).
        // However, in order to persist a HGRoleConstruct, we need a graph to lookup handles of player and type, so it has to be provided externally.
        setHyperGraph(((HGTopicMapSupport)((TopicMapImpl)owner.getTopicMap()).getSupport()).getGraph());
    ...
    protected Role createOwner() {
        ...
        RoleImpl role = new RoleImpl(parent.getTopicMapSupport().getOwner(), (AssociationImpl) parent.getOwner());
    ...
    public void setPlayer(TopicImpl player) {
        ...
        if (thisHandle != null) {
            // If setPlayer is called when the role is being created, it won't be in the graph (yet) and therefore update will fail.
            // If setPlayer is called to update an existing role, on the other hand, we need to let the graph know.
