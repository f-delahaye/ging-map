...
public class HGNameSupport extends HGScopedSupport<Name> implements NameSupport {
    ...
    // For compliance with Javabeans standard (which allows NameSupport to be persisted as a JavabeansType)
    ...
    public Set<Variant> getVariants() {
        final HGHandle handle = getHandle(hyperGraph, this);
        return handle == null ? null :  HGTMUtil.<HGVariantSupport>getRelatedObjects(hyperGraph, HGTM.hVariantOf, null, handle).stream().map(support->support.getOwner()).collect(Collectors.toSet());
    }
