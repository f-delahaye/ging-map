...
public class IMTopicSupport extends IMConstructSupport implements TopicSupport {  
    ...
    public Set<Role> getRolesPlayed() {
        ...
        // Role.id may be modified by role.importIn() and role.id is used in hashCode() which could lead to inconsistent behavior of roles (as a general rule, its not safe to put mutable objects in Set's)
        // In particular, one of the tests of the standard TMAPI test suite fails because of this
        // (I suspect IMAssociationSupport.getRoles() would also fail in IMAssociationSupport.getRoles().contains(xxx) was tested)
        ...
        // TODO if we create a new HashSet here, we no longer need to wrap result of this method in Collections.unmodifiableSet
